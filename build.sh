...
UBOOT_REPO=https://github.com/seebe/u-boot-2015.01_hachiko.git
KERNEL_REPO=https://github.com/seebe/linux-3.14_hachiko.git
...
function usage {
  ...
  echo -e "    ./build.sh kernel hachiko_xip_defconfig : Switch to XIP version of the kernel"
...
Examples:
  ...
  # Device Tree Blob
  ./build.sh jlink output/linux-3.14/arch/arm/boot/dts/r7s72100-hachiko.dtb 0x180C0000

  # Kernel
  ./build.sh jlink output/linux-3.14/arch/arm/boot/uImage 0x20000000
...
if [ "$1" == "kernel" ] || [ "$1" == "k" ] ; then
  ...
  if [ ! -e linux-3.14 ] ;then
    echo "cloning kernel..."
    git clone $KERNEL_REPO linux-3.14
  fi
...
#  fi
  ...
  # Patch kernel
  if [ ! -e arch/arm/configs/hachiko_defconfig ] ;then
  ...
  if [ "$IMG_BUILD" == "1" ] ; then
    ...
    #       the command line
    echo LOCALVERSION= -e "make -j$BUILD_THREADS $2 dtbs\n"
...
if [ "$1" == "u-boot" ] || [ "$1" == "u" ] ; then
  ...
  if [ ! -e u-boot-2015.01 ] ;then
    echo "cloning u-boot..."
    git clone $UBOOT_REPO u-boot-2015.01
  fi
...
#  fi
  ...
  # Patch u-boot
  if [ ! -e include/configs/hachiko.h ] ;then
    # Combine all the patches, then patch at once
...
if [ "$1" == "buildroot" ]  || [ "$1" == "b" ] ; then
 ...
 # Patch and Configure Buildroot for the HACHIKO
  if [ ! -e configs/hachiko_defconfig ]; then
    ...
    # Copy in our default Buidlroot config
    # NOTE: It was made by running this inside buildroot
    #   make savedefconfig BR2_DEFCONFIG=../../patches-buildroot/hachiko_defconfig
    # or rather
    #   ./build.sh buildroot savedefconfig BR2_DEFCONFIG=../../patches-buildroot/hachiko_defconfig
    ...
    echo "======================================================================="
    echo ""
    echo "Are you going to use video (LCD)?"
    echo "If you do not need LCD, then we will not build those packages"
    echo "and that will make building faster."
    echo ""
    echo "What type of file system do you prefer?"
    echo ""
    echo "1 Video pacakges (default)"
    echo "2 Minimum system"
    echo ""
    for i in 1 2 3 ; do
    ...
    if [ "$ANSWER" == "1" ] ; then
      # Select our full HACHIKO config
      make hachiko_defconfig
    fi

    if [ "$ANSWER" == "2" ] ; then
      make hachiko_min_defconfig
    fi
...
if [ "$1" == "axfs" ] ; then
  ...
  if [ ! -e axfs/mkfs.axfs ] ; then
    ...
    # Add execute permission (sometimes gets lost in clone or unzip)
    chmod +x mkfs.axfs
...
###############################################################################
if [ "$1" == "gdb" ] ; then
  banner_yellow "Starting GDB"

  # Check if JLinkGDBServer running
  if [ "$(ps -e | grep JLinkGDBServer)" == "" ] ; then
    echo -e "\n\tERROR - JLinkGDBServer is not running\n"
    echo -e "\tSTART GDB:  JLinkGDBServer -speed 15000 -if JTAG -device R7S721001"
    exit
  fi

  DTB=r7s72100-hachiko.dtb
  $OUT=.
  # xipImage or uImage?
  if [ $OUT/arch/arm/boot/xipImage -nt $OUT/arch/arm/boot/uImage ] ; then
    IMAGE=xipImage
  else
    IMAGE=uImage
  fi

#  if [ ! -e .gdbinit ] ; then # Only write once
  if [ "1" == "1" ] ; then    # write out every time
    echo "
define kload_xip
	...
	# Run the code till start_kernel
	#info address start_kernel
	set \$i = (int)start_kernel
	tb *\$i
	#c
	echo Ready\n
end
...
define kload
	...
	echo Copying updated binaries...\n
	shell cp arch/arm/boot/$IMAGE /tmp/$IMAGE.bin
	shell cp arch/arm/boot/dts/$DTB /tmp/$DTB.bin
	...
	echo Loading files...\n
	monitor loadbin /tmp/$DTB.bin,0x09800000
	monitor loadbin /tmp/$IMAGE.bin,0x09000000
	file vmlinux
	...
	#c
	echo Ready\n
	#shell echo -e \"Ready. Now type Continue\r\n\"
end

target remote localhost:2331
monitor reset

" > $OUTDIR/linux-3.14/.gdbinit
  fi

  cd $OUTDIR/linux-3.14

  # GDB location settings
  GDB_PATH=$OUTDIR/buildroot-$BR_VERSION/output/host/usr/bin
